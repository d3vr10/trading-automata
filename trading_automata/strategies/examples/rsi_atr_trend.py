...
class RSIATRTrendStrategy(BaseStrategy):
    ...
    def on_bar(self, bar: Bar) -> Optional[Signal]:
        ...
        if signal:
            ...
            logger.info(
                f"EURUSDStrategy signal for {bar.symbol}: {signal} "
    ...
    def _check_signal_cooldown(self, symbol: str) -> bool:
        ...
        bars_since_signal = len(self.price_history[symbol]) - \
                           (list(self.price_history[symbol]).index(
                               self.price_history[symbol][-1]) if self.last_signal_time[symbol] else 0)

        # Simplified: use deque length as proxy for time
        if len(self.price_history[symbol]) < self.signal_cooldown:
            return False
    ...
    def _generate_signal(self, bar: Bar, indicators: Dict[str, Any]) -> Optional[Signal]:
        ...
        # Buy Signal: RSI oversold + Bullish trend
        if rsi < self.rsi_oversold and trend:
            return Signal(
                symbol=bar.symbol,
                action='buy',
        ...
        # Sell Signal: RSI overbought + Bearish trend
        elif rsi > self.rsi_overbought and not trend:
            return Signal(
                symbol=bar.symbol,
                action='sell',
                quantity=self.position_size,
